		...
		fs.readFile(config.cwd + config.folder + file, 'utf8', function (err, data) {
				if (err) {
					...
					data = data.replace(requireMatch, function(pattern, path){
						var fullPath  = _path.join(_path.resolve(config.cwd), config.folder, path);
					...
					var folder = _path.join(_path.resolve(config.cwd), config.folder);
					if(config.packedFolder){
						folder = _path.join(folder, config.packedFolder);
					...
					//write new file
					fs.writeFile(_path.join(folder, fileName), data, function (err) {
						if (err){
